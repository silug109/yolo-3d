...
def metrics_numpy_new(model, directory = None, anchors=anchors_list):
    ...
    for file in full_dir_list['pathname_base']:
        ...
        frame, targets, y_true = process_oneframe(file)
        y_pred = model.predict(frame)
        cell_x = np.reshape(np.tile(np.arange(max_grid_d), max_grid_h * max_grid_w),
                            (max_grid_h, max_grid_d, max_grid_w, 1, 1))
        ...
        true_wh = np.exp(true_box_wh) * anchors[..., :N_DIM]
